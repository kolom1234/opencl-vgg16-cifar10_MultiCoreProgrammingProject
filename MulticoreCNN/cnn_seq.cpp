...
void writeFile(const char* path, float** data, int i) {
	int len = OUTPUT_DIM[i] * NBYN[i] * NBYN[i];
	float* layer = data[i];
	...
	// layer 배열의 데이터를 파일에 씁니다.
	size_t written = fwrite(layer, sizeof(float), len, file);
	if (written != len) {
		perror("파일 쓰기에 실패했습니다");
	}
...
void cnn_seq(float* images, float* network, int* labels, float* confidences, int num_of_image) {
	...
	for (int i = 0; i < 21; ++i) {
		// 파일 이름을 "layer{index}.txt" 형식으로 생성합니다.
		char filename[20];
		snprintf(filename, sizeof(filename), "layer%d.txt", i);

		// 파일에 레이어 데이터를 씁니다.
		writeFile(filename, layer, i);
	}
